...
const NewScribble: NextPageWithLayout = () => {
  ...
  return (
    <>
      <NextSeo noindex nofollow title="New Scribble" />
      {loading && (
      ...
      {!loading && user && (
        <ScribbleForm userId={user.id} username={user.username ?? ""} />
      )}
    </>
  );
};

const ScribbleForm: React.FC<{ userId: string; username: string }> = ({
  userId,
  username,
}) => {
  ...
  const onSubmit = async (data: { title: string; emoji: string }) => {
    ...
    if (newPost.data) {
      ...
      router.push(`/${username}/scribbles/${slug}`);
    }
      ...
      <Container className="pt-12 sm:pt-20" size="small">
        <div className="text-center flex flex-col">
          ...
          <p className="text-tMuted text-sm">
            ...
            thread format. It can be used not only for recording development but
            also for sharing info and exchanging opinions.
          </p>
        </div>
